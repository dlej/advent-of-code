...
# we need only check 3 bits at a time of A, starting with the
# leading bits which correspond to the last outputs
for i in range(1, len(program) + 1):
    next_candidates = []
    for a in candidates:
        for j in range(8):
            if comp.run(program, a=8 * a + j, b=0, c=0) == program[-i:]:
                next_candidates.append(8 * a + j)
